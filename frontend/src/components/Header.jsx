  ...
  return (
    <>
      <header
        className={`${
          location.pathname === "/"
            ? "absolute bg-[#00000000]"
            : "sticky bg-black"
        } top-0 hidden min-h-20 z-50 lg:flex w-full items-center py-2 justify-between px-16`}
      >
        <div className="flex items-center gap-2">
          <NavLink to={"/"} className={"flex items-center gap-1 mx-4"}>
            <img
              src="https://res.cloudinary.com/duhmeadz6/image/upload/v1731116089/logo_hdf34r.png"
              alt=""
              width={30}
            />
            <span className="text-2xl font-bold">BitShare</span>
          </NavLink>
          <nav>
        ...
        <NavLink
          to={"/send"}
          className="bg-white text-black rounded-full text-sm py-2 px-3 font-normal hover:bg-opacity-90"
        >
          Transfer now
        </NavLink>
      </header>
      <header
        ...
        } top-0 w-full flex items-center justify-between lg:hidden z-50`}
      >
        <NavLink to={"/"} className={"flex items-center gap-1 mx-4"}>
          <img
            src="https://res.cloudinary.com/duhmeadz6/image/upload/v1731116089/logo_hdf34r.png"
        ...
        {showSideBar ? (
          ...
          >
            <IoMdCloseCircleOutline
              size={"2.5rem"}
              className="text-white-light mx-4"
            />
          </button>
        ) : (
          <button
        ...
        <div
          ...
          } bg-black absolute flex flex-col items-center justify-around top-16 z-50 transition-transform origin-top`}
          style={{ height: `calc(100vh - 5rem)`, minWidth: "100vw" }}
        >
          <nav>
